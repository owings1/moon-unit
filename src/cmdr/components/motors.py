...
class MotorScripts:
  ...
  def perflib_build(self):
    ...
    with lib.while_vargt(0, 0):
      lib.add(':set_var*', 2, moic.GlobalVarPtr[0])                 #   20,  16,  12,   8,  4
      lib.add(':var_math2', 2, moic.Math2Oper.MATH2_MUL, 50)  # 1000, 320, 240, 160, 80
      lib.add(':set_var', 1, 8)
      with lib.while_vargt(1, 0):
        lib.add('move*', moic.GlobalVarPtr[2])
        lib.add('move_rev*', moic.GlobalVarPtr[2])
        lib.add('delay*', moic.GlobalVarPtr[0])
        lib.add('move*', moic.GlobalVarPtr[2])
        lib.add('move_rev*', moic.GlobalVarPtr[2])
        lib.add('move*', moic.GlobalVarPtr[0])
        lib.add('move_rev*', moic.GlobalVarPtr[0])
        lib.add('delay*', moic.GlobalVarPtr[0])
        lib.add('move*', moic.GlobalVarPtr[0])
        lib.add('move_rev*', moic.GlobalVarPtr[0])
        lib.add('move*', moic.GlobalVarPtr[1])
        lib.add('delay*', moic.GlobalVarPtr[1])
        lib.add('move_rev*', moic.GlobalVarPtr[1])
